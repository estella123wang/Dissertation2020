...
ro_tec = readRDS("~/hda_tds_Mechanomics_project/Proteins/Proteins_technical_covariates.rds")
...
library(data.table)
univ_name<-setDT(univ0, keep.rownames = TRUE)[]
...
plot(univ$coef, -log10(as.numeric(univ$pval)), pch = 19,
  ...
  ylab = expression(-log[10](p[value])), col = ifelse(univ$pval<0.1, yes = "tomato",no = "darkgrey"))
  
if (!is.null(annot)) {
    #annot = annot[rownames(univ0), ]
    text(univ0$coef, -log10(univ$pval), pos = 3,offset = 0.2, cex = 0.5,
         labels=ifelse(univ$pval < thr, yes = annot$rn, no = ""))
}

  abline(v = 0, lty = 3)
  
  abline(h = -log10(0.05/nrow(univ)), lty = 2,col = "darkred")

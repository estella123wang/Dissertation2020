...
```{r}
proteins1<-readRDS("~/dissertation/dissertation/data/Proteins.rds")
covars1<-readRDS("~/dissertation/dissertation/data/Covariates1.rds")
...
MysPLSDA_pooled <- splsda(X_pooled, Y_pooled, ncomp = 1, mode="regression", keepX = 3)#best: num = 3
MygPLSDA_pooled <- gPLSda(X_pooled, Y_pooled, ncomp =1,
                          ind.block.x = Xgroups,
                          keepX=1)
MysgPLSDA_pooled <- sgPLSda(X_pooled, Y_pooled, ncomp = 1,
ind.block.x = Xgroups, keepX =1, alpha.x =0.5)#group 1, alpha = 0.5                          
```
...
axis(1, at = seq(1, 85 * 7, by = 7), labels = colnames(MyPLSDA_pooled$X), las = 2)
...
```{r}
MSEP_sPLSDA = NULL
for (subtype in c(NA,"Large-cell carcinoma", "Other lung cancer", "Adenocarcinoma", "Squamous-cell carcinoma", "Small-cell carcinoma")) {

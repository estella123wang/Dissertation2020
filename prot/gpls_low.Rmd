...
```{r}
set.seed(1)
res_sgplsda = CalibratesgPLSDA(dataX = X_pooled, dataY = Y_pooled, ncomp = 1, Nrepeat = 20, Xgroups = Xgroups)
```
...
```{r}
MysgPLSDA_pooled <- sgPLSda(X_pooled, Y_pooled, ncomp = 1, ind.block.x = Xgroups, keepX = 3, alpha.x = 0.9)
```
...
pdf("sgPLS and sPLSDA_low.pdf",width = 30, height = 8)
...
axis(1, at = c(0, Xgroups, 92) * 4, line = 6, labels = NA) 
...
abline(v = c(0, Xgroups, 92) * 4, lty = 3, col = "black") 
abline(h = 0, lty = 2)
...
setwd("~/dissertation/dissertation/prot/pls")
pdf("misclassification_gPLS_low.pdf")
...
```{r}
#c(22, 67, 82)
sum(MysgPLSDA_pooled$loadings$X[82:92]>0)
```

...
```{r}
X_pooled<-high_ttd[,1:92]

Y_pooled = high_ttd[,93]
```
...
```{r}
set.seed(1)
res_splsda = CalibratesPLSDA(dataX = X_pooled, dataY = Y_pooled,ncomp = 1, Nrepeat = 50)
```
...
```{r}
MysgPLSDA_pooled <- sgPLSda(X_pooled, Y_pooled, ncomp = 1, ind.block.x = Xgroups, keepX = 3, alpha.x = 0.7)
```
...
pdf("sgPLS and sPLSDA_high.pdf",width = 30, height = 8)
...
setwd("~/dissertation/dissertation/prot/pls")
pdf("misclassification_gPLS_high.pdf")
...
```{r}
#c(22, 67, 82)
sum(MysgPLSDA_pooled$loadings$X[67:82]<0)
```
